-- Script para corrigir funções de ranking

-- Remover funções existentes para evitar conflitos
DROP FUNCTION IF EXISTS public.check_table_exists(text, text);
DROP FUNCTION IF EXISTS public.get_question_attempts_ranking();
DROP FUNCTION IF EXISTS public.get_question_attempts_ranking_by_period(text);

-- Criar função para verificar se uma tabela existe
CREATE OR REPLACE FUNCTION public.check_table_exists(
    in_table_name TEXT, 
    in_schema_name TEXT DEFAULT 'public'
)
RETURNS BOOLEAN AS $$
DECLARE
    v_exists BOOLEAN;
BEGIN
    SELECT EXISTS (
        ...
        WHERE t.table_schema = in_schema_name
...
CREATE OR REPLACE FUNCTION public.get_question_attempts_ranking_by_period(in_period TEXT)
RETURNS TABLE (
    ranking_position INTEGER,
    user_id UUID,
    display_name TEXT,
