...
serve(async (req) => {
  ...
  try {
    ...
    const { pdfBase64, mimeType, authorName } = await req.json();
...
7. Crie um conteúdo HTML bem formatado com parágrafos, títulos e listas quando apropriado`;
    ...
    const postData = {
      ...
      tags: parsedContent.tags.split(',').map((tag: string) => tag.trim()),
      meta_description: parsedContent.metaDescricao,
      meta_keywords: parsedContent.palavrasChave.split(',').map((keyword: string) => keyword.trim()),
      featured: false,
      is_draft: false,
      comment_count: 0,
      likes_count: 0,
    ...
    console.error('Erro na edge function create-blog-post-from-pdf:', error);
    return new Response(
      JSON.stringify({ 
