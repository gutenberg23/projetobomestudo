...
export const QuestionFooter: React.FC<QuestionFooterProps> = ({
  ...
  hasTeacherExplanation = false,
  hasAIExplanation = false,
  isSubmittingAnswer = false
}) => {
  return (
    <footer className="flex flex-wrap justify-between items-center py-5 px-2 md:px-6 w-full text-sm md:text-base font-bold text-center gap-2">
      <button 
      ...
      >
        <span className="flex flex-1 shrink gap-1 md:gap-2.5 justify-center items-center py-1 md:py-2.5 pr-1 md:pr-8 pl-1 md:pl-8 basis-0 px-1 md:px-[10px]">
          <img 
            ...
            alt="Answer Icon" 
            className="object-contain shrink-0 self-stretch my-auto aspect-square w-[14px] md:w-[19px]" 
          />
      ...
      >
        <span className="flex flex-1 shrink gap-1 md:gap-2.5 justify-center items-center py-1 md:py-2.5 basis-0 px-1 md:px-[10px]">
          <img 
            ...
            alt="Messages Icon" 
            className="object-contain shrink-0 self-stretch my-auto w-3 md:w-4 aspect-square" 
          />
          <span className="text-xs md:text-base">
      ...
      {hasAIExplanation && (
        <button 
          ...
          className={`flex shrink-0 justify-center items-center px-4 py-3 md:py-4 rounded-xl border border-purple-400 hover:bg-purple-50 transition-colors ${showAIAnswer ? "text-gray-700 bg-purple-100" : "text-purple-400 bg-white"}`}
        >
