...
export const CourseNavigation = ({
  ...
  return (
    <nav className="bg-[rgba(246,248,250,1)] flex min-h-[92px] w-full items-center justify-between gap-5 text-[17px] text-slate-500 overflow-x-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-slate-400 px-[10px] md:px-[32px]">
      ...
      {activeTab === 'disciplinas' && (
        <button 
          ...
          className={cn("hidden md:flex items-center gap-2.5 px-5 py-4 rounded-[10px] hover:bg-white transition-colors whitespace-nowrap ml-auto", isProgressVisible && "bg-white text-[#5f2ebe]")}
        >
