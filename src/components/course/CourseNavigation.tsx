...
export const CourseNavigation = ({
  ...
  return <nav className="bg-[rgba(246,248,250,1)] flex min-h-[92px] w-full items-center justify-between gap-5 text-[17px] text-slate-500 overflow-x-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:hover:bg-slate-400 px-[32px]">
      <div className="flex items-center gap-5">
        ...
        <button onClick={() => setActiveTab('simulados')} className={cn("flex items-center gap-2.5 px-5 py-4 rounded-[10px] hover:bg-white transition-colors whitespace-nowrap", activeTab === 'simulados' && "bg-white text-[#f11ce3]")}>
          <Activity className="w-6 h-6" />
          <span className={cn("md:inline text-xs md:text-base", activeTab === 'simulados' ? "inline" : "hidden")}>Simulados</span>
        </button>
      </div>

      {activeTab === 'disciplinas' && <button onClick={onProgressClick} className={cn("hidden md:flex items-center gap-2.5 px-5 py-4 rounded-[10px] hover:bg-white transition-colors whitespace-nowrap ml-auto", isProgressVisible && "bg-white text-[#f11ce3]")}>
