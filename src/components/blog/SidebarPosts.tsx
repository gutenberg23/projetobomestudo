...
export const SidebarPosts: React.FC<SidebarPostsProps> = ({
  posts,
  title,
  icon
}) => {
  if (!posts.length) return null;
  return <div className="bg-white rounded-lg shadow-sm p-5 mb-6">
      ...
      <div className="space-y-4">
        {posts.map(post => <div key={post.id} className="flex gap-3 group">
            {post.featuredImage && <Link to={`/blog/${post.slug}`} className="shrink-0">
            ...
            <div>
              <Link to={`/blog/${post.slug}`}>
                <h4 className="font-medium text-[#272f3c] line-clamp-2 group-hover:text-[#5f2ebe] transition-colors">
                  {post.title}
                </h4>
              </Link>
