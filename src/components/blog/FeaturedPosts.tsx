...
export const FeaturedPosts: React.FC<FeaturedPostsProps> = ({
  ...
  return <div className="grid md:grid-cols-2 gap-6 mb-10">
      ...
      <div className="relative group">
        <Link to={`/blog/${mainPost.slug}`}>
          <div className="relative aspect-[16/9] rounded-lg overflow-hidden mb-4">
            <img src={mainPost.featuredImage || "/placeholder.svg"} alt={mainPost.title} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-300" />
      ...
      <div className="space-y-6">
        {secondaryPosts.map(post => <div key={post.id} className="flex gap-4 group">
            <Link to={`/blog/${post.slug}`} className="shrink-0">
              <div className="relative w-32 h-24 overflow-hidden rounded-lg">
                <img src={post.featuredImage || "/placeholder.svg"} alt={post.title} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-300" />
              </div>
            </Link>
            <div className="flex-1">
              <div className="inline-block bg-[#ede7f9] text-[#5f2ebe] text-xs font-medium px-2 py-1 rounded-md mb-1">
                {post.category}
              </div>
        ...
        <Link to="/blog" className="flex items-center text-[#ea2be2] hover:text-[#5f2ebe] font-medium">
