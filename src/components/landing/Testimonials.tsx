...
export const Testimonials = () => {
  return <div className="w-full px-2.5 py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto">
          ...
          image: "https://i.pravatar.cc/100?img=3"
        }].map((testimonial, index) => <div key={index} className="bg-white p-8 rounded-xl shadow-sm hover:shadow-md transition-shadow">
              <div className="flex gap-1 mb-4">
                {[1, 2, 3, 4, 5].map(star => <Star key={star} className="w-4 h-4 fill-[#5f2ebe] text-[#5f2ebe]" />)}
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-[#5f2ebe] text-sm">{testimonial.role}</div>
                </div>
              </div>
            </div>)}
        </div>
