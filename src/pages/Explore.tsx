...
const ResultItem: React.FC<ItemProps> = ({
  ...
  return <div className="flex justify-between items-center p-4 border-b border-gray-100 hover:bg-gray-50">
      ...
      <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(id)} className="ml-4" aria-label={isFavorite ? "Remover dos favoritos" : "Adicionar aos favoritos"}>
...
const Explore = () => {
  ...
  return <div className="flex flex-col min-h-screen bg-[f6f8fa] bg-[#f6f8fa]">
      <Header />
      <main className="flex-grow pt-[120px] px-4 md:px-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
          <div className="flex items-center flex-1 max-w-3xl relative">
            <Input type="text" placeholder="Pesquisar..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pr-10" />
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="divide-y divide-gray-100">
            {filteredData.length > 0 ? filteredData.map(item => <ResultItem key={item.id} id={item.id} title={item.title} description={item.description} isFavorite={item.isFavorite} onToggleFavorite={handleToggleFavorite} />) : <div className="p-8 text-center text-gray-500">
